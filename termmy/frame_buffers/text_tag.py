...
class TextTag(Iterable):
    ...
    def position(self) -> Vec2i:
        return Vec2i(self._position.x + len(self._text) // 2, self._position.y)
    
    def starting_position(self) -> Vec2i:
    ...
    def centered(self, is_centered:bool) -> None:
        ...
        if self.centered == is_centered:
            ...
            self._position = Vec2i(
                self._position.x - len(self._text) // 4,
                self._position.y
            )
    ...
    def text(self, text:str = ""):
        ...
        self._position = Vec2i(self._position.x + diff, self._position.y)
    
    @position.setter
    def position(self, position:Vec2i):
    ...
    def __iter__(self):
        color_code = self.color.to_ansi_txt_24() if self.color else ""
        return (f"{color_code}{self._text[i:i+2]}"
